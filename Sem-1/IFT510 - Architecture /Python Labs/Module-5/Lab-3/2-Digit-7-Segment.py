...
def display_two_digits(canvas, x, y, width, height, digit1, digit2, outline_width=1, spacing=10):
    canvas.delete("all")
    draw_digit(canvas, x, y, width, height, digit1, outline_width)
    draw_digit(canvas, x + width + spacing, y, width, height, digit2, outline_width)

# Test the display sequence using a simple for loop
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Two-Digit Seven-Segment Display")
    ...
    for digit1 in range(10):  # Display the first digit from 0 to 9
        for digit2 in range(10):  # Display the second digit from 0 to 9
            display_two_digits(canvas, 10, 10, (canvas_width - 30) // 2, (canvas_height - 30) // 4, digit1, digit2, outline_width)
            root.update()  # Update the GUI to show the new digits
            root.after(delay_time)  # Delay before displaying the next digits
